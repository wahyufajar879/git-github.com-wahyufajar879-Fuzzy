...
include 'includes/tsukamoto.php';
include 'includes/tsukamoto_pakar.php';

$mod = $_GET['m'];
$act = $_GET['act'];

$db->query("DELETE FROM tb_himpunan WHERE kode_kriteria NOT IN (SELECT kode_kriteria FROM tb_kriteria)");
$db->query("DELETE FROM tb_aturan WHERE kode_kriteria NOT IN (SELECT kode_kriteria FROM tb_kriteria)");
$db->query("DELETE FROM tb_aturan WHERE kode_himpunan NOT IN (SELECT kode_himpunan FROM tb_himpunan)");
